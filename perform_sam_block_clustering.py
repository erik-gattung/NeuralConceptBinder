...
def get_args():
	...
	args.topk = 5
	# args.retrieval_corpus_path = os.path.join("/app", *args.checkpoint_path.split("/")[2:-1], "block_concept_dicts.pkl")
	args.retrieval_corpus_path = "/app/ncb/CLEVR-4/retbind_seed_1/block_concept_dicts.pkl"
	args.retrieval_encs = "proto-exem"
	args.deletion_dict_path = None
	args.merge_dict_path = None
...
def slots_to_blocks(slots, num_frames, num_objects, args):
	"""Reshape the slot encodings to block encodings."""
	slot_size = 256
	num_blocks = 8
	assert slot_size/num_blocks == int(slot_size/num_blocks)
	return torch.reshape(slots, (num_frames, num_objects, num_blocks, int(slot_size/num_blocks)))


def gather_obj_encs(model, base_dir, video_range, args, sea_raft_model, sea_raft_args, sam2_predictor):
	...
	for video_num in video_range:
		if video_num % 50 == 0:
			print(f"------------- At video {video_num} -------------")
		img_locs = sorted(glob.glob(os.path.join(base_dir, f"video_{video_num}", "*.jpg")))

		transform = transforms.Compose([
			transforms.ToTensor()  # Converts image to [C, H, W] tensor in range [0, 1]
		])
		imgs = torch.stack([transform(Image.open(filename).convert("RGB")) for filename in img_locs])

		# encode image, return it ordered after frame, object
		pointers_array = model.encode_cont(img_locs, sea_raft_model, sea_raft_args, sam2_predictor).transpose(1, 0, 2)
		num_frames, num_objects = pointers_array.shape[0:2]

		block_encs = slots_to_blocks(torch.Tensor(pointers_array), num_frames, num_objects, args)

		# with SAM2 we have a dynamic amount of "slots", but all capture objects
		valid_values = False
		for i in range(num_objects):
			if torch.isfinite(block_encs[:, i]).all():
				all_encs.append(block_encs[::15, i].detach().cpu().numpy())	# only take in every 20th image to reduce cluster size
				valid_values = True
...
def main():
	...
	for block_id in range(num_blocks):
		...
		clusterer = perform_cluster_per_block_precomputed(train_block_encs, block_id=block_id, args=args, reduce_dim=-1,
														  n_param_steps=0, verbose=0, metric='cosine')
	...
	# save all_img_locs as pickle file
