...
class SAMConceptBinder(nn.Module):
    ...
    def encode(self, imgs: list[str], sea_raft_model, sea_raft_args, sam2_predictor):
        # B = imgs.shape[0]
        # imgs = imgs.to(self.device)


        # slots, attns_vis, attns, _ = self.model.encode(imgs)
        object_array, pointers_array, inference_state, image_filenames = sam2_pipeline_main.main(imgs, sea_raft_model, sea_raft_args, sam2_predictor, top_k_zero_crossings=10)

        print(pointers_array.shape)
        ...
        print("starting discrete retrieval")

        # TODO: get retrieval corpus for 256 dim
        representations = torch.stack(
            [self.retrieve_discrete_representation(torch.from_numpy(obj_ptrs).cuda()) for obj_ptrs in pointers_array]
        )

        print("finished discrete retrieval")

        hard_codes = representations[..., 0]
    ...
    def encode_cont(self, imgs: list[str], sea_raft_model, sea_raft_args, sam2_predictor):
        object_array, pointers_array, inference_state, image_filenames = sam2_pipeline_main.main(imgs, sea_raft_model, sea_raft_args, sam2_predictor, top_k_zero_crossings=10)
